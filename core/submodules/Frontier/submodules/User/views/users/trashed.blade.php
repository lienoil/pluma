                    ...
                    <v-toolbar class="transparent elevation-0">
                        ...
                        <v-slide-y-transition>
                            <template v-if="dataset.selected.length > 1">
                                <form :action="route(urls.users.restore, false)" method="POST" class="inline">
                                    {{ csrf_field() }}
                                    {{ method_field('PATCH') }}
                                ...
                                <v-dialog transition="scale-transition" persistent v-model="dataset.dialog.model" lazy width="auto">
                                    ...
                                    <v-card class="elevation-4 text-xs-center">
                                        ...
                                        <v-card-actions class="pa-3">
                                            ...
                                            <v-spacer></v-spacer>
                                            <form :action="route(urls.users.delete, false)" method="POST" class="inline">
                                                {{ csrf_field() }}
                                                {{ method_field('DELETE') }}
                                                <template v-for="item in dataset.selected">
                                                <input type="hidden" name="id[]" :value="item.id">
                                ...
                                <v-menu bottom left>
                                    ...
                                    <v-list>
                                        <v-list-tile ripple @click="$refs[`restore_${prop.item.id}`].submit()">
                                            ...
                                            <v-list-tile-content>
                                                <v-list-tile-title>
                                                    <form :id="`restore_${prop.item.id}`" :ref="`restore_${prop.item.id}`" :action="route(urls.users.restore, prop.item.id)" method="POST">
                                                        {{ csrf_field() }}
                                                        {{ method_field('PATCH') }}
                                                        {{ __('Restore') }}
                                                    </form>
                                                ...
                                                <v-card class="text-xs-center elevation-4">
                                                    ...
                                                    <v-card-actions class="pa-3">
                                                        ...
                                                        <form
                                                            ...
                                                            :action="route(urls.users.delete, prop.item.id)" method="POST">
                                                                {{ csrf_field() }}
                                                                {{ method_field('DELETE') }}
