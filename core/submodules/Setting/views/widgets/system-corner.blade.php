    ...
    >
        <v-slide-y-transition>
            <v-card v-show="!removesystemcorner" transition="slide-y-transition" class="elevation-1 mb-3 draggable-widget">
                <v-card-media height="150px" src="{{ assets('frontier/images/placeholder/9.png') }}">
                ...
                <v-card-text v-show="!hidesystemcorner">
                    <v-subheader>{{ __('Server Environment') }}</v-subheader>

                    <div class="body-1 pa-3">
                        <div class="body-2">{{ __('Server Software') }}</div>
                        <div>{{ $_SERVER['SERVER_SOFTWARE'] }}</div>
                    </div>

                    <div class="body-1 pa-3">
                        <div class="body-2">{{ __('Server Admin') }}</div>
                        <div>{{ $_SERVER['SERVER_ADMIN'] }}</div>
                    </div>

                    <div class="body-1 pa-3">
                        <div class="body-2">{{ __('Document Root') }}</div>
                        <div>{{ $_SERVER['DOCUMENT_ROOT'] }}</div>
                    </div>

                    <div class="body-1 pa-3">
                        <div class="body-2">{{ __('Remote Address') }}</div>
                        <div>{{ $_SERVER['REMOTE_ADDR'] }}</div>
                    </div>

                    <div class="body-1 pa-3">
                        <div class="body-2">{{ __('Server Signature') }}</div>
                        <div>{!! $_SERVER['SERVER_SIGNATURE'] !!}</div>
                    </div>
                </v-card-text>
                <v-card-actions v-show="!hidesystemcorner">
                    <v-spacer></v-spacer>
                    <v-btn flat href="{{ route('settings.system') }}">{{ __('View More') }}</v-btn>
                </v-card-actions>
            </v-card>
        </v-slide-y-transition>
    </draggable>
