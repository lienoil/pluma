<select {{ $attr ?? '' }} title="{{ __('Icon') }}" name="{{ $name ?? 'details[#][icon]' }}" class="form-control">
  <optgroup label="{{ __('Misc') }}">
    ...
    <option {{ isset($value) ? ($value === 'fa fa-birthday' ? 'selected="selected"' : '') : '' }} value="fa fa-birthday-cake" data-icon="fa fa-birthday-cake">{{ __('Birthday') }}</option>
    <option {{ isset($value) ? ($value === 'fe fe-map' ? 'selected="selected"' : '') : '' }} value="fe fe-map-pin" data-icon="fe fe-map-pin">{{ __('Map Pin') }}</option>
