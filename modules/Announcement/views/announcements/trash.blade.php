...
@section("head-title", __('Trashed Announcements'))
@section("page-title", __('Trashed Announcements'))
            ...
            <v-flex xs12>
                <v-card class="mb-3">
                    <v-toolbar class="transparent elevation-0">
                        <v-toolbar-title>{{ __('Trashed Announcements') }}</v-toolbar-title>
                        <v-spacer></v-spacer>
                        ...
                        <v-slide-y-transition>
                            <template v-if="dataset.selected.length > 1">
                                <div>
                                    {{-- Bulk Restore --}}
                                    <form action="{{ route('announcements.many.restore') }}" method="POST" class="inline">
                                        {{ csrf_field() }}
                                    ...
                                    <v-dialog v-model="dataset.dialog.model" lazy width="auto">
                                        ...
                                        <v-card>
                                            ...
                                            <v-card-actions>
                                                ...
                                                <v-spacer></v-spacer>
                                                <form action="{{ route('announcements.many.delete') }}" method="POST" class="inline">
                                                    {{ csrf_field() }}
                                                    {{ method_field('DELETE') }}
                                                    <template v-for="item in dataset.selected">
                                                        <input type="hidden" name="announcements[]" :value="item.id">
                        ...
                        <template slot="items" scope="prop">
                            ...
                            <td class="text-xs-center">
                                <v-menu bottom left>
                                    ...
                                    <v-list>
                                        <v-list-tile ripple @click="post(route(urls.announcements.api.restore, (prop.item.id)))">
                                            <v-list-tile-action>
    ...
    <v-dialog v-model="resource.dialog.model" persistent lazy width="auto" min-width="200px">
        <v-card class="text-xs-center">
            ...
            <v-card-actions>
                ...
                <v-spacer></v-spacer>
                <form :action="route(urls.announcements.delete, (resource.dialog.data.id))" method="POST" class="inline">
                    {{ csrf_field() }}
                    {{ method_field('DELETE') }}
...
@push('pre-scripts')
    ...
    <script>
        ...
        mixins.push({
            data () {
                return {
                    ...
                    urls: {
                        announcements: {
                            api: {
                                restore: '{{ route('api.announcements.restore', 'null') }}',
                                delete: '{{ route('api.announcements.delete', 'null') }}',
                            },
            ...
            methods: {
                get () {
                    this.api().get('{{ route('api.announcements.all') }}', this.dataset.pagination)
                        .then((data) => {
                            this.dataset.items = data.items.data ? data.items.data : data.items;
                            this.dataset.totalItems = data.items.total ? data.items.total : data.total;
                            this.dataset.loading = false;
