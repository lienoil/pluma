        ...
        <v-layout row wrap>
                ...
                <form action="{{ route('forums.categories.store', 'forums') }}" method="POST">
                    ...
                    <v-card class="elevation-1">
                        <v-toolbar flat class="transparent">
                            <v-icon>label</v-icon>
                            <v-toolbar-title class="subheading">{{ __("New Forum Category") }}</v-toolbar-title>
                        </v-toolbar>
                        <v-card-text>
                            <v-text-field
            ...
            <v-flex sm9>
                ...
                <v-card class="mb-3 elevation-1">
                    <v-toolbar flat class="transparent">
                        <v-toolbar-title class="subheading">{{ __('All Forum Categories') }}</v-toolbar-title>
                        <v-spacer></v-spacer>
        ...
        mixins.push({
            data () {
                return {
                    ...
                    urls: {
                        categories: {
                            edit: '{{ route('forums.categories.edit', 'null') }}',
                            destroy: '{{ route('forums.categories.destroy', 'null') }}',
                        }
                    },
            ...
            watch: {
                ...
                'dataset.searchform.query': function (filter) {
                    setTimeout(() => {
                        ...
                        this.api().search('{{ route('api.categories.search', 'forums') }}', query)
                            .then((data) => {
            ...
            methods: {
                get () {
                    ...
                    this.api().get('{{ route('api.categories.all', 'forums') }}', query)
                        .then((data) => {
                            this.dataset.items = data.items.data ? data.items.data : data.items;
                            this.dataset.totalItems = data.items.total ? data.items.total : data.total;
                            this.dataset.loading = false;
