      ...
      <v-flex lg3 md4>
            <v-card class="elevation-1 c-lift" height="100%">
              <v-layout column wrap fill-height class="ma-0">
                <a class="td-n" href="{{ route('courses.show', $course->slug) }}">
                  <v-card-media class="accent lighten-3" src="{{ $course->backdrop }}" height="220px">
                              ...
                              <v-card flat class="transparent">
                                  <v-card-actions class="py-3">
                                      <v-avatar class="elevation-4"  size="80px">
                                          <img src="{{ $course->feature }}" alt="{{ $course->title }}">
                                      </v-avatar>
                                  </v-card-actions>
                              </v-card>
                          </v-layout>
                ...
                {{-- Title --}}
                <v-card-title>
                    <button href="{{ route('courses.show', $course->slug) }}" >{{ $course->title }}</button>
                </v-card-title>
              </v-layout>
          </v-card>
      </v-flex>
