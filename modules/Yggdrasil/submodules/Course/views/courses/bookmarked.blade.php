...
@section("main-content")
    <v-toolbar dark class="mb-2 primary elevation-1 sticky">
        <v-icon dark>bookmark</v-icon>

        <v-toolbar-title>
            @if (request()->all())
                {{ __('Bookmarked Filtered Courses') }}
            @else
                {{ __('Bookmarked Courses') }}
            @endif
        </v-toolbar-title>
    </v-toolbar>
    ...
    <v-container fluid grid-list lg>
        <v-layout row wrap fill-height>
            <template>
                <v-flex xs12>
                    <v-card flat class="transparent grey--text text-xs-center">
                        <v-card-text>
                            <div>
                                <v-card-text class="pa-5">
                                    <img
                                        class="empty states--opacity"
                                        width="120px"
                                        src="{{ assets('frontier/images/empty_states/data-table.svg') }}"
                                        alt="">
                                </v-card-text>
                            </div>
                            <div class="mb-1">
                                <strong>{{ __('No resource found.') }}</strong>
                            </div>
                            <div class="my-3">
                                <v-btn primary href="{{ route('courses.all') }}">{{ __('Browse Courses') }}</v-btn>
                            </div>
                        </v-card-text>
                    </v-card>
                </v-flex>
                    ...
                    <v-flex lg3 md4>
                        <v-card class="elevation-1 c-lift" height="100%">
                            <v-layout column wrap fill-height class="ma-0">
                                ...
                                <v-card-title>
                                    <a href="{{ route('courses.single', $resource->slug) }}" class="accent--text td-n"></a>
                                </v-card-title>
                                <v-card-actions>
