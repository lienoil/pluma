                    ...
                    <v-toolbar class="transparent elevation-0">
                        ...
                        <v-slide-y-transition>
                            <template v-if="dataset.selected.length > 1">
                                <form :action="route(urls.courses.restore, false)" method="POST" class="inline">
                                    {{ csrf_field() }}
                                    {{ method_field('PATCH') }}
                        ...
                        <v-slide-y-transition>
                            <template v-if="dataset.selected.length > 1">
                                {{-- Delete --}}
                                <form :action="route(urls.courses.delete, false)" method="POST" class="inline">
                                    {{ csrf_field() }}
                                    {{ method_field('DELETE') }}
                                    <template v-for="item in dataset.selected">
                                        <input type="hidden" name="id[]" :value="item.id">
                                ...
                                <v-menu bottom left>
                                    ...
                                    <v-list>
                                        <v-list-tile ripple @click="$refs[`restore_${prop.item.id}`].submit()">
                                            ...
                                            <v-list-tile-content>
                                                <v-list-tile-title>
                                                    <form :id="`restore_${prop.item.id}`" :ref="`restore_${prop.item.id}`" :action="route(urls.courses.restore, prop.item.id)" method="POST">
                                                        {{ csrf_field() }}
                                                        {{ method_field('PATCH') }}
                                                        {{ __('Restore') }}
                                                    </form>
                                        ...
                                        <v-list-tile ripple @click="$refs[`delete_${prop.item.id}`].submit()">
                                            ...
                                            <v-list-tile-content>
                                                <v-list-tile-title>
                                                    <form :id="`delete_${prop.item.id}`" :ref="`delete_${prop.item.id}`" :action="route(urls.courses.delete, prop.item.id)" method="POST">
                                                        {{ csrf_field() }}
                                                        {{ method_field('DELETE') }}
        ...
        mixins.push({
            data () {
                return {
                    ...
                    urls: {
                        courses: {
                            restore: '{{ route('courses.restore', 'null') }}',
                            delete: '{{ route('courses.delete', 'null') }}',
                        }
                    },
                    dataset: {
                        headers: [
